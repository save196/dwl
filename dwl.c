...
assignkeymap(struct wlr_keyboard *keyboard) {
	...
	if (*kblayout_file && (f = fopen(kblayout_file, "w"))) {
		fputs(xkb_keymap_layout_get_name(keyboard->keymap, keyboard->modifiers.group), f);
		fclose(f);

		// Run kblayout_cmd
		if (kblayout_cmd[0])
			spawn(&(Arg){ .v = (const void *)kblayout_cmd });
	}
...
tile(Monitor *m)
{
	unsigned int h, r, mw, my, tw = 0, tx = 0, ty;
	...
	if (n > m->nmaster) {
		mw = m->nmaster ? (int)roundf(m->mfact * (m->w.width - 3 * gaps)) : 0;
		tw = m->w.width - mw - 2 * gaps;
		tx = mw + gaps;
		if (m->nmaster) {
			tw -= gaps;
			tx += gaps;
	...
	wl_list_for_each(c, &clients, link) {
		...
		if (i < m->nmaster) {
			r = MIN(n, m->nmaster) - i;
			h = (m->w.height - my - gaps * r) / r;
			resize(c, (struct wlr_box){.x = m->w.x + gaps, .y = m->w.y + my,
				.width = mw, .height = h}, 0);
			my += c->geom.height + gaps;
		} else {
			r = n - i;
			h = (m->w.height - ty - gaps * r) / r;
			resize(c, (struct wlr_box){.x = m->w.x + tx, .y = m->w.y + ty,
				.width = tw, .height = h}, 0);
